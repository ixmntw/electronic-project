...
$sql1="DELETE FROM issue_out WHERE i_id='$_REQUEST[i_id]'";
mysql_query($sql1,$conn) or die ("Unable to connect to the database.").mysql_error() ;

$sql2="DELETE FROM issue WHERE i_id='$_REQUEST[i_id]'";
mysql_query($sql2,$conn) or die ("Unable to connect to the database.").mysql_error() ;
mysql_close($conn);
?>
<script language="javascript">
alert("Delete Done.");
window.location.href="borrow.php?i_id=<?=$_REQUEST[i_id]?>";
</script>
<? }else{ ?>
<script language="javascript">
