...
<p>
  <?  	
require ('connect.php');
$strSQL ="SELECT * FROM  issue WHERE  id='$rs[id]' and (i_id LIKE '%".$_GET["txtKeyword"]."%') order by i_id DESC ";
...
if($_GET["txtKeyword"] != "")
	// Search By Name 
 require ('connect.php');
	$strSQL = "SELECT * FROM  issue WHERE  id='$rs[id]' and (i_id LIKE '%".$_GET["txtKeyword"]."%') order by i_id DESC LIMIT $Page_Start , $Per_Page  ";
	$objQuery = mysql_query($strSQL) or die ("Error Query [".$strSQL."]");
	$Total_amount=0;
	?>
</p>
 ...
 <table width="85%" border="0" align="center" cellpadding="0" cellspacing="0" class="ex-lgradient3">
   <tr >
     <td height="38" align="center"><h1 class="style11">ข้อมูลการยืม-คืนอุปกรณ์</h1>     </td>
   </tr>
   <tr>
     <td align="center" valign="bottom"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" id="customers" class="style10">
       ...
       <tr>
         ...
         <td width="11%" align="center"><a href="borrow_detail.php?i_id=<?=$info[i_id]?>">
           <?=$info[i_id];?>
         </a></td>
         <td width="11%" align="center"><?=$info[i_date];?></td>
         <td width="12%" align="center"><?=$info[i_date_return_plan];?></td>
         <td width="11%" align="center"><? if($info[i_status]==0){ echo "รออนุมัติ";}
         ...
         <td width="3%" height="41" align="center" valign="middle" bordercolor="#CCCCCC"><? if($info[i_status]==0 or $info[i_status]==2){?>
           <a href="borrow_edit.php?i_id=<?=$info[i_id]?>" class="style12"> <img src="img/edit.gif" width="25" border="0" /></a>
             <? }?>         </td>
         <td width="3%" align="center" valign="middle" bordercolor="#CCCCCC"><? if($info[i_status]==0 or $info[i_status]==2){?>
           <script language="JavaScript" type="text/javascript">
             ...
             <a href="borrow_del.php?i_id=<?=$info[i_id]?>&status=<?=$info[i_status]?>" class="style2="style2="style2""" OnClick="return chkdel();"><img src="img/dele.gif" width="30" border="0" /></a>
             <? }?>         </td>
         <td width="3%" align="center" valign="middle" bordercolor="#CCCCCC"><? if($info[i_status]==1){?>
           <a href="borrow_return.php?i_id=<?=$info[i_id]?>&amp;status=<?=$info[i_status]?>"> <img src="img/2031484.png" width="30" height="30" border="0" /></a>
             <? } ?></td>
         <td width="3%" align="center" valign="middle" bordercolor="#CCCCCC"><? if($info[i_status]==1){?>
          <a href="borrow_print.php?i_id=<?=$info[i_id]?>"><img src="img/Print-icon.png" width="30" border="0" /></a>		 <? } ?></td>
       </tr>
       <? }?>
     </table>
     <p>&nbsp; </p></td>
