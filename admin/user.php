...
<p>
  <?  	
require ('connect.php');
$strSQL ="SELECT * FROM user WHERE  (username LIKE '%".$_GET["txtKeyword"]."%' or name LIKE '%".$_GET["txtKeyword"]."%') order by username ASC ";
...
if($_GET["txtKeyword"] != "")
	// Search By Name 
 require ('connect.php');
	$strSQL = "SELECT * FROM user WHERE  (username LIKE '%".$_GET["txtKeyword"]."%' or name LIKE '%".$_GET["txtKeyword"]."%') order by username ASC  LIMIT $Page_Start , $Per_Page  ";
	$objQuery = mysql_query($strSQL) or die ("Error Query [".$strSQL."]");
	$Total_amount=0;
	?>
</p>
