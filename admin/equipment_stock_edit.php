...
<p>
  <?  require ('connect.php');
	...
	$strSQL = "SELECT * FROM equipment inner join stock on equipment.e_id=stock.e_id WHERE  stock.in_id ='$in_id' ";
  ...
  <table width="85%" border="0" align="center" cellpadding="0" cellspacing="0" class="ex-lgradient2">
    <tr >
      ...
      <td width="96%" align="center"><h1 class="style11">แก้ไขสต๊อกอุปกรณ์อิเล็กทรอนิกส์</h1></td>
    </tr>
    <tr>
      <td colspan="2" align="center" valign="bottom"><form action="equipment_stock_edit_save.php" method="post" enctype="multipart/form-data"name="form1" id="form1"equipment_stock_edit_save.php="equipment_stock_edit_save.php">
        <h3>&nbsp;</h3>
          ...
          <tr>
            <td align="center" class="ex-lgradient">จำนวน
              <input type="hidden" name="e_id" value="<?=$info[e_id];?>" /> 
			   <input type="hidden" name="in_id" value="<?=$info[in_id];?>" />
           </td>
            <td align="center"><input name="qty" type="text" class="select" style="text-align:center;" value="<?=$info[in_qty];?>"/></td>
          </tr>
        </table>
		<br/>
