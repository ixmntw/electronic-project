...
<p>
  <?  	
require ('connect.php');
$strSQL ="SELECT * FROM  issue inner join user on issue.id=user.id 
WHERE  issue.i_date_return='0000-00-00' and i_status='1' and (issue.i_id LIKE '%".$_GET["txtKeyword"]."%') order by issue.i_id DESC ";
...
if($_GET["txtKeyword"] != "")
	// Search By Name 
 require ('connect.php');
	$strSQL = "SELECT * FROM  issue inner join user on issue.id=user.id WHERE issue.i_date_return='0000-00-00' and i_status='1' and (issue.i_id LIKE '%".$_GET["txtKeyword"]."%') order by issue.i_id DESC LIMIT $Page_Start , $Per_Page  ";
	$objQuery = mysql_query($strSQL) or die ("Error Query [".$strSQL."]");
	$Total_amount=0;
	?>
</p>
 ...
 <table width="85%" border="0" align="center" cellpadding="0" cellspacing="0" class="ex-lgradient3">
   <tr >
     <td height="38" align="center"><h1 class="style11">ข้อมูลบุคลากรที่ยังไม่ได้คืนอุปกรณ์</h1>
     </td>
   </tr>
   <tr>
     <td align="center" valign="bottom"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" id="customers" class="style10">
         <tr class="ex-lgradient">
           ...
           <td width="30%" align="center">ชื่อ-นามสกุลผู้ยืม</td>
         ...
         <tr>
           ...
           <td width="30%" align="center"><?=$info[name];?>
               <?=$info[lname];?></td>
